...
import '../baseModel/trainingGoal_model.dart';
...
// 6.2 날짜가 2일이상 지난 항목들을 fail 상태로 바꾸기
  ...
  //     SET tg_success = 2
  //     WHERE tg_duedate < date('now','-2 days') AND tg_success = 0
  //     ''';
// ** Sid를 받아서 해당 종목의 모든 목표들을 가져오는 기능들
//     7. 달성중인 목표들 가져오는 기능
  final String _getProGoalListBySidStr =
      "SELECT * FROM training_goal WHERE tg_success = 0 AND tg_s_id = ? ORDER BY tg_priority ASC, tg_duedate ASC, tg_insertdate ASC;";
//     8. 성공한 목표들 가저오는 기능
  final String _getSuccessGoalsBySidStr =
      "SELECT * FROM training_goal WHERE tg_success =1 AND tg_s_id = ? ORDER BY tg_duedate DESC LIMIT ?";
//     9. 실패한 목표들 가져오는 기능
  final String _getFailedGoalsBySidStr =
      "SELECT * FROM training_goal WHERE tg_success =2 AND tg_s_id = ? ORDER BY tg_duedate DESC LIMIT ?";
//     10. duedate 가 지난 목표들 가져오는 기능
