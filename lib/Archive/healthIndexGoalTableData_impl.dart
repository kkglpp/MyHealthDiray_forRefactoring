...
// 3. healthIndex goal을 새로 입력하는 구문
  final String _insertGoal_str = '''INSERT INTO hi_goal(
  hg_height,
  hg_weight,
  hg_fat,
  hg_muscle,
  ...
  Future<List<HealthIndexGoalModel>> getGoalList() async {
    ...
    try {
      final List<Map<String, dynamic>> result =
          await db.rawQuery(_getGoalList_str);
...
// 2. healthIndex goal 1개를 가져오는 함수
  Future<HealthIndexGoalModel?> getGoal(int id) async {
    ...
    try {
      final List<Map<String, dynamic>> result =
          await db.rawQuery(_getGaol_str, [id]);
...
// 5. healthIndex goal을 삭제하는 함수.
  ...
  Future<bool> deleteGoal(int id) async {
    ...
    try {
      result = await db.rawDelete(_deleteGoalStr, [id]);
    } catch (e) {
      return false;
    }
