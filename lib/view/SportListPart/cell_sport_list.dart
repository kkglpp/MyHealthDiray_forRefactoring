...
class CellSportList extends ConsumerWidget {
  ...
  final String opt;
  final int folderID;  
  const CellSportList({super.key, required this.sportId, required this.sportName, required this.width, required this.eachheight, required this.opt, required this.folderID});
    ...
    //폴더별 리스트 새로고침
      ...
      child: SizedBox(
        ...
        child: WidgetCustomTextBox(
          msg: sportName,
          fontSize: sportName.length > 7 ? fontSize(context, 1.5) : fontSize(context, 2),
          verAlign: 1,
        ),
      ),
    );
