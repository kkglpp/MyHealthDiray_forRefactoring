...
import 'package:myhealthdiary_app/provider/sort_folder_state.dart';
import 'package:myhealthdiary_app/provider/sport_folder_list_state_notifier.dart';
...
import '../../managerClass/sportfolder_list_manager.dart';
import '../../managerClass/sport_list_manager.dart';
...
class SportListView extends ConsumerWidget {
  ...
  final String opt;
  const SportListView( {super.key,this.opt ="goal",});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      //스포츠 전체 목록을 관리하는 부분
      final sportList = ref.watch(wholeListStateProvider);
      //폴더명들을 관리하는 부분
                ...
                child: Row( //전체를 관통하는 row
                  ...
                  children: [
                    SizedBox(
                      ...
                      width: cellWidth,
                      // height: boxHeight,
                      child: Column( //리스트 전체를 보여주기위한 column
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          children: [
                            Column(
                              children: [
                                GestureDetector( //폴더명 길게 클릭하면 폴더 지우기
                                  onLongPress: () async{
                                    bool confirm = await baseAlertForConfirm(context, "폴더를 지우시겠습니까?");
                                    if(confirm){
                                      //삭제하고
                                      ref.read(folderStateForDeleteProvider(folder.sfId!).notifier).deleteFolder();
                                      //폴더리스트 초기화
                                      ref.read(folderListProvider.notifier).setState();
                                    }
                                  },
                                  child: Container( // 폴더명 들어가는 박스
